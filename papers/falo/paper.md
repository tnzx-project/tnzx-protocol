...
1. **Zero-knowledge group membership.** Members prove they belong to the group without revealing which member they are, using Merkle tree inclusion proofs.

2. **Blind vouching.** New members are admitted by existing members who do not learn the new member's identity, and vice versa.

3. **Proof of Time.** A novel Sybil resistance mechanism that requires sustained mining activity over 30 days, replacing monetary stake (which excludes those who most need the tool).

4. **Emergent location.** Meeting coordinates are computed as a deterministic function of all participants' encrypted contributions — no one can predict the location before the event.

5. **Honest threat modeling.** Explicit, user-facing documentation of what Falo can and cannot protect against.
...
| `MIN_HASHES` | 1,000,000 | ~100 hours CPU mining |
| `MIN_CONSISTENCY` | 70% | Active 70% of days in the period |
| `MAX_ANOMALY_SCORE` | 30 | No suspicious behavioral patterns |

### 5.3 Properties

**Time cannot be bought.** Unlike financial stake, 30 days of mining history cannot be purchased, transferred, or fabricated. It requires actual sustained computation over calendar time.
...
Falo uses VS2 [1] as its primary transport, inheriting all its properties:

- **Steganographic.** All Falo traffic travels hidden in mining share submissions
- **Mining-gated.** Only active miners can send/receive Falo messages
- **Self-funding.** Mining fees fund the relay infrastructure
- **Undetectable.** An observer sees mining traffic, nothing more
...
### 9.4 User-Facing Warning

The system displays on first use:
...
4. The system is fully transparent — all specifications are published
